...
import { CHROME_WEB_STORE_URL, SUPABASE } from './config.js';
...
// Cache sites loaded for the galaxy so the wander fallback can use them
...
async function handleWanderClick() {
    const btn = elements.wanderBtn;
    if (btn) {
        btn.disabled = true;
        btn.textContent = 'Finding a site…';
    }

    let url = null;
    try {
        const res = await fetch(
            `${SUPABASE.url}/rest/v1/community_sites?select=url,upvote_count,downvote_count`,
            {
                headers: {
        ...
        if (res.ok) {
            ...
            const eligible = (data || []).filter(
                (row) => (row.upvote_count || 0) >= (row.downvote_count || 0) && row.url
            );
            if (eligible.length) {
                url = eligible[Math.floor(Math.random() * eligible.length)].url;
            }
        }
    } catch (e) {
        console.warn('Wander: Supabase fetch failed, using fallback:', e);
    }

    if (!url && loadedSites?.length) {
        const site = loadedSites[Math.floor(Math.random() * loadedSites.length)];
        url = site?.url;
    }

    if (btn) {
        btn.disabled = false;
        btn.textContent = 'Wander the Web';
    }
...
function initEventListeners() {
    ...
    if (elements.galaxyBtn) {
        elements.galaxyBtn.addEventListener('click', handleGalaxyClick);
    }
